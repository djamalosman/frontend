...
class NewsUpdateController extends Controller
{
    ...
    public function getContentNewsList (Request $request)
    {
        ...
        if (!empty($filters['jenisberitatop']) && is_array($filters['jenisberitatop'])) {
            $whereData->whereIn('m_news.nama', $filters['jenisberitatop']);
            
        } elseif (!empty($filters['jenisberitatop']) && is_string($filters['jenisberitatop'])) {
            $whereData->where('m_news.nama', 'LIKE', '%' . $filters['jenisberitatop'] . '%');
        }
        ...
        if (!empty($filters['sortBy'])) {
            switch ($filters['sortBy']) {
                case 'newest':
                    $whereData->orderBy('news_detail.created_at', 'desc');
                    break;
                case 'oldest':
                    $whereData->orderBy('news_detail.created_at', 'asc');
                    break;
                //case 'rating':
            ...
            $whereData->orderBy('news_detail.created_at', 'desc'); // Default sort
        }
    ...
    public function detailNews($id)
    {
        ...
        $data['listfiles']=  NewsFileModel::where('id_news_dtl',$id)->get();
        $query = DB::table('news_detail')
        ->leftJoin('m_news', 'm_news.id', '=', 'news_detail.id_m_news')
        ->select(
            'news_detail.*',
        ...
        $whereData=$query->where('news_detail.status',1);
        $data['getdataDetail']=$whereData->where('news_detail.id',$id)->first();
